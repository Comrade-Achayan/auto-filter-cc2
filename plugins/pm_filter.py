    ...
    if 0 < len(message.text) < 50:    
        ...
        if files:
            for file in files:
                ...
                filename = f"[{get_size(file.file_size)}] {file.file_name}"
                btn.append(
                    [InlineKeyboardButton(text=f"{filename}", url=f"https://telegram.dog/{nyva}?start=cinema_collections_-_-_-_{file_id}")]
                )
        else:
            return
        if not btn:
